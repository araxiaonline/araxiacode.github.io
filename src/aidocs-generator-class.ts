    ...
    if (ts.isClassDeclaration(node) && node.name) {
      ...
      node.members.forEach((member) => {
        if (ts.isMethodDeclaration(member) && member.name) {
          ...
          let comments = getLeadingComments(member); // Extract comments
          classMethods.push({ className, methodName, methodSignature, comments }); // Add comments to the object
        }
      });
    }
...
async function createDocs(classMethods: ClassMethod[], aiModel: AiModel): Promise<void> {
    const fewShotExample = fs.readFileSync(`${__dirname}/player.ts`);
